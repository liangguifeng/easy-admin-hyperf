            ...
            <div class="select-options">
                <ul>
                    @foreach($selector['options'] as $value => $option)
                        @php
                            $active = in_array($value, \Hyperf\Utils\Arr::get($selected, $column, []));
                        @endphp
                        <li>
                            <a href="{{ \Admin\Grid\Tools\Selector::url($column, $value, true) }}"
                               class="{{$active ? 'active' : ''}}">{{ $option }}</a>
                            @if(!$active && $selector['type'] == 'many')
                                &nbsp;
                                <a href="{{ \Admin\Grid\Tools\Selector::url($column, $value) }}" class="add"><i
                    ...
                    @endforeach
                    <li>
                        <a href="{{ \Admin\Grid\Tools\Selector::url($column) }}" class="clear"><i
